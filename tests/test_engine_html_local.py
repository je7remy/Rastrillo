...
class TestRecipeAutoGenerated(IsolatedTestCase):
    ...
    def test_verify_usa_expect_gone_cuando_hay_click_css(self):
        """El verify sintetizado prefiere expect_gone con el primer selector
        CSS clicado (más útil que success_selector=body)."""
        log = [
            {"turn":0,"action":{"action":"click","selector":"button#delete",
                                "reason":"abrir modal"},
             "result":{"ok":True}},
            {"turn":1,"action":{"action":"click","selector":"#confirm",
                                "reason":"confirmar"},
             "result":{"ok":True}},
        ]
        out = self.recipes_auto.synthesize_and_save(
            host="z.com", start_url="https://z.com/", agent_log=log,
            result_status="deleted")
        self.assertIsNotNone(out)
        _, recipe = out
        verify = recipe["steps"][-1]
        self.assertEqual(verify["action"], "verify")
        self.assertEqual(verify.get("expect_gone"), "button#delete")
        self.assertNotIn("success_selector", verify)

    def test_verify_fallback_cuando_solo_text_clicks(self):
        """Si todos los clicks fueron por texto (text=...), no podemos usar
        expect_gone sobre ese 'selector' textual; caemos al fallback body."""
        log = [
            {"turn":0,"action":{"action":"click","text":"Delete"},
             "result":{"ok":True}},
        ]
        out = self.recipes_auto.synthesize_and_save(
            host="q.com", start_url="https://q.com/", agent_log=log,
            result_status="deleted")
        self.assertIsNotNone(out)
        _, recipe = out
        verify = recipe["steps"][-1]
        self.assertEqual(verify.get("success_selector"), "body")
        self.assertNotIn("expect_gone", verify)
